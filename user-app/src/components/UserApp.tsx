...
const ReorderModal = React.memo(({
  ...
  const getSelectedItems = () => {
    ...
    return order.itemDetails.filter((item, index) =>
      ...
      const reorderItem = reorderItems.find((r) => r.name === item.name);
...
const FadedSkiesApp = () => {
    ...
    if (isAuthenticated && user.email) {
      try {
        ...
        const handleOrderUpdate = (update: any) => {
          ...
          showToastMessageFunc(`Order ${update.orderId}: ${update.message || 'Status updated'}`);
        };
        ...
        const handleOrderAccepted = (data: any) => {
          ...
          showToastMessageFunc(`Great news! Driver ${data.driverName} accepted your order and is on the way!`);
        };
        ...
        const handleOrderStatusUpdate = (data: any) => {
          ...
          showToastMessageFunc(data.message || `Order ${data.orderId} status updated`);
        };
    ...
    if (authForm.email && authForm.email.trim() && authForm.password && authForm.password.trim()) {
      try {
        ...
        if (response.success) {
          ...
          showToastMessageFunc('Welcome back to Faded Skies!');
        } else {
        ...
        try {
          ...
          if (mockResponse.success) {
            ...
            showToastMessageFunc('Welcome back to Faded Skies! (Demo Mode)');
          } else {
    ...
    const apiCall = async () => {
      try {
        ...
        if (response.success) {
          ...
          setCurrentView('home');
          showToastMessageFunc('Account created successfully! Welcome to Faded Skies!');
        } else {
        ...
        try {
          ...
          if (mockResponse.success) {
            ...
            setCurrentView('home');
            showToastMessageFunc('Account created successfully! Welcome to Faded Skies! (Demo Mode)');
          } else {
      ...
      try {
        ...
        try {
          ...
          if (response.success) {
            ...
            showToastMessageFunc('Order placed successfully! Admin will confirm shortly.');
          } else {
            console.warn('Backend order submission failed:', response.error);
            showToastMessageFunc('Order placed! Processing with backup system.');
          }
        } catch (apiError) {
          console.warn('API submission failed, using local processing:', apiError);
          showToastMessageFunc('Order placed successfully! Preparing your delivery.');
        }
  ...
  // Function to show toast messages (alias for showToastMessage)
  const showToastMessageFunc = showToastMessage;

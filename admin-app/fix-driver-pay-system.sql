...
CREATE TRIGGER calculate_driver_pay_trigger
    AFTER INSERT OR UPDATE OF distance, tip ON orders
    FOR EACH ROW
    EXECUTE FUNCTION trigger_calculate_driver_pay();

-- Update existing orders with calculated driver pay
UPDATE orders SET 
...
VALUES (
    'SCHEMA_UPDATE',
    'orders',
    'Added driver pay columns: driver_base_pay, driver_mileage_pay, driver_total_pay. Updated pay rates to $2.00 base + $0.70/mile',
    NOW()
);
